...
function LogstashAgent() {
  ...
  this.on('alarm', function(mode, module) {
    if (mode) {
      this.alarm_count += 1;
      logger.warn('Alarm on for module', module, 'number of alarms', this.alarm_count);
      if (this.closed_inputs === false) {
        logger.warn('Alarm, closing inputs plugins');
        this.close_inputs();
    ...
    if (!mode) {
      this.alarm_count -= 1;
      logger.warn('Alarm off for module', module, 'number of alarms', this.alarm_count);
      if (this.alarm_count === 0 && this.closed_inputs === true) {
        logger.warn('Alarm off, starting inputs plugins');
        this.start_inputs();
...
LogstashAgent.prototype.configure = function(url, type, callback) {
  ...
  try {
    logger.info(__('Initializing module: %s', type));
    ...
    // Save output module for dynamic lookup (allows dynamic reconfiguration).
    if (type === 'output') {
      this.outputs[parsed.protocol] = module;
    } else if (type === 'filter') {
      this.filters[parsed.protocol] = module;
    }
...
LogstashAgent.prototype.getOutputModule = function(protocol) {
  if (this.outputs.hasOwnProperty(protocol)) {
    return this.outputs[protocol];
  }
  return null;
};

LogstashAgent.prototype.getFilterModule = function(protocol) {
  if (this.filters.hasOwnProperty(protocol)) {
    return this.filters[protocol];
  }
  return null;
};
